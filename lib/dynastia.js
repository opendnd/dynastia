...
class Dynastia {
  ...
  constructor(opts = {}) {
    ...
    this.defaultsPersonae = opts.defaultsPersonae;

    // initialize personae
    this.personae = new Personae({
      defaults: this.defaultsPersonae,
  ...
  validateOpts(opts = {}) {
    ...
    if (!this.themes.includes(opts.theme)) opts.theme = this.themes.sample();
    ...
    // name
    if ((opts.name === undefined) || (opts.name === '')) opts.name = this.nomina.generate({ theme: opts.theme, gender: 'male' });
  ...
  // simulate progenitor
  simulateProgenitor(startingYear = 0, progenitor) {
    const { husbandSeed, wifeSeed } = this.opts;
  ...
  // try a million simluations until one works out (hacky but better than recursive)
  ...
  generateGeneration(generationsCount = 0, startingYear = 0, progenitor, inheritance) {
    generationsCount -= 1;
    let consort;
    let regnant;
    let heir;
    ...
    const simulation = this.trySimluationsOfProgenitor(startingYear, progenitor, inheritance);
    ...
    if (consort) {
      ...
      if (heir && (generationsCount > 0)) {
        generation.heir = this.generateGeneration(generationsCount, heir.birthYear, heir, inheritance);
      }
    }
  ...
  generate(opts = {}) {
    ...
    const generations = this.generateGeneration(generationsCount, startingYear, progenitor, inheritance);
