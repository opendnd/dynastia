...
if (program.input) {
  ...
  const person = Personae.load(program.input);
  const { theme, DNA } = person;
  const { gender, race, uuid } = DNA;
  ...
  const diff = person.birthYear - lastRegnant.birthYear;

  // remove the last regnant and add the person
  ...
  Saver.finish(outputDir, 'Would you like to save your ancestors? (y | n)', dynasty, person.name);
