    ...
    Object.keys(chromosomes).forEach(function (index) {
      ...
      if (size === 'sex') {
        ...
        if (gender === 'male') {
          ...
          if (father === undefined) {
            fatherPair = roll.roll('1' + y).result;
          } else {
            fatherPair = father.DNA[index].y;
          }
        ...
        // assign keys if we are male, otherwise it doesn't matter
        if (gender === 'male') {
          pairs[index] = { y: fatherPair, x: motherPair };
        } else {
