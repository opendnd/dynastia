...
class Simulator {
  ...
  // generate lifespan age
  static generateLifeAge(race = 'Dragonborn', gender = 'female') {
  ...
  generateChild(mother, father) {
    const { theme } = mother;
    const race = mother.DNA.race;
    const gender = defaults.genders.sample();
    ...
    const deathYear = birthYear + child.age;
  ...
  generateSpouse(person) {
    ...
    // get seeds
    if ((gender === 'male') && husbandSeed) spouseOpts.seed = husbandSeed;
    if ((gender === 'female') && wifeSeed) spouseOpts.seed = wifeSeed;

    let spouse = this.personae.generate(spouseOpts);
