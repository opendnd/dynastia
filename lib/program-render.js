...
if (program.input) {
  ...
  if (program.interface) {
    ...
    const displayPersonList = (err, res) => {
      term.clear();
      term.white('Please select a person from the Dynasty below for more information:\n\n');
      const items = Object.values(persons).map((person) => colors.white(Renderer.name(person, yearFormat)));
