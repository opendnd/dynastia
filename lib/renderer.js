...
class Renderer {
  ...
  static name(person) {
    ...
    return `${name} ${Renderer.gender(gender)} (${Renderer.year(birthYear)} - ${Renderer.year(deathYear)})`;
  }
  ...
  static outputGeneration(generation, dynasty) {
    ...
    if (regnant && consort) {
      ...
      output += `     Married : ${Renderer.year(regnant.marriageYear)}\n`;
      ...
      if (regnant.issue.length > 0) {
        ...
        regnant.issue.forEach((childID) => {
          ...
          // if we have a successor check if this child is it
          if (generation.successor) {
            if (childID === generation.successor.regnant) {
              bullet = '*';
              isSuccessor = true;
            }
          }
          ...
          if (isSuccessor) {
            output += colors.yellow.bold(`${prepend}${bullet} ${Renderer.name(child)}\n`);
          } else {
            output += `${prepend}${bullet} ${Renderer.name(child)}\n`;
          }
        });
      }
    }
    if (generation.successor) output += Renderer.outputGeneration(generation.successor, dynasty);
